...
- Required fields: `title`, `startDate` (or `date`), `startTime` (or `time`), `desc`.
- Optional fields: `endDate` (defaults to start date), `endTime` (defaults to `23:59`).
- `organisers` is optional.
- If an image is attached, it is downloaded by the bot and sent to API as structured image payload.
- Event `startDate + startTime` must be in the future, otherwise creation is rejected.
...
- Default rotation is `event 1 → calendar → event 2 → calendar → …`. Set `DISPLAY_CALENDAR_EVERY_X_EVENTS` to a positive integer to show the calendar after that many event slides (default `1`; invalid values fall back to `1`). The count continues across event-list wraparound, so `3` means three event slides even when fewer than three distinct events exist. With no events the calendar stays visible; `/calendar` is always calendar-only regardless of this setting.
...
- The calendar screen contains only the card grid: no heading, date, count, clock, or update/status banner. The clock returns on community-event slides.
- With no community events, the calendar stays on screen. With no remaining Momence sessions, it displays a minimal end-of-day message. On a schedule fetch failure, it retains the last successful data, still removes started sessions, and retries automatically. A minimal loading/error message appears only when no schedule has loaded.
