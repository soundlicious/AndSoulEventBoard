...
- Default rotation is `event 1 → calendar → event 2 → calendar → …`. Set `DISPLAY_EVERY_X_EVENTS` to a positive integer to show the calendar after that many event slides (default `1`; invalid values fall back to `1`). The count continues across event-list wraparound, so `3` means three event slides even when fewer than three distinct events exist. With no events the calendar stays visible; `/calendar` is always calendar-only regardless of this setting.
